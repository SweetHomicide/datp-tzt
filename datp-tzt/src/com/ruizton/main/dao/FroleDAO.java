...
public class FroleDAO extends HibernateDaoSupport {
	...
	public Frole findById(java.lang.String id) {
		log.debug("getting Frole instance with id: " + id);
		try {
			Frole instance = (Frole) getSession().get("com.ruizton.main.model.Frole", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Frole> findByExample(Frole instance) {
		...
		try {
			List<Frole> results = (List<Frole>) getSession()
					.createCriteria("com.ruizton.main.model.Frole").add(create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Frole instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Frole as model where model."
					+ propertyName + "= ?";
	...
	public List<Frole> list(int firstResult, int maxResults, String filter,
			boolean isFY) {
