...
public class FquestionDAO extends HibernateDaoSupport {
	...
	public Fquestion findById(java.lang.String id) {
		log.debug("getting Fquestion instance with id: " + id);
		try {
			Fquestion instance = (Fquestion) getSession().get("com.ruizton.main.model.Fquestion",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fquestion> findByExample(Fquestion instance) {
		...
		try {
			List<Fquestion> results = (List<Fquestion>) getSession()
					.createCriteria("com.ruizton.main.model.Fquestion").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fquestion instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Fquestion as model where model."
					+ propertyName + "= ?";
