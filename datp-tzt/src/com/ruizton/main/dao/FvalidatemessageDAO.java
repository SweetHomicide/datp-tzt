...
public class FvalidatemessageDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FvalidatemessageDAO.class);
	// property constants
	...
	public Fvalidatemessage findById(java.lang.String id) {
		log.debug("getting Fvalidatemessage instance with id: " + id);
		try {
			Fvalidatemessage instance = (Fvalidatemessage) getSession().get(
					"com.ruizton.main.model.Fvalidatemessage", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fvalidatemessage> findByExample(Fvalidatemessage instance) {
		...
		try {
			List<Fvalidatemessage> results = (List<Fvalidatemessage>) getSession()
					.createCriteria("com.ruizton.main.model.Fvalidatemessage")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fvalidatemessage instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fvalidatemessage as model where model."
					+ propertyName + "= ?";
	...
	public Fvalidatemessage merge(Fvalidatemessage detachedInstance) {
		...
		try {
			Fvalidatemessage result = (Fvalidatemessage) getSession().merge(
					detachedInstance);
