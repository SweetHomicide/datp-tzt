...
public class FlogDAO extends HibernateDaoSupport {
	...
	public Flog findById(java.lang.Integer id) {
		...
		try {
			Flog instance = (Flog) getSession().get("com.ruizton.main.model.Flog", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Flog> findByExample(Flog instance) {
		...
		try {
			List<Flog> results = (List<Flog>) getSession()
					.createCriteria("com.ruizton.main.model.Flog").add(create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Flog instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Flog as model where model."
					+ propertyName + "= ?";
