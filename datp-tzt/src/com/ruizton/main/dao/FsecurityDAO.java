...
public class FsecurityDAO extends HibernateDaoSupport {
	...
	public Fsecurity findById(java.lang.String id) {
		log.debug("getting Fsecurity instance with id: " + id);
		try {
			Fsecurity instance = (Fsecurity) getSession().get("com.ruizton.main.model.Fsecurity",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fsecurity> findByExample(Fsecurity instance) {
		...
		try {
			List<Fsecurity> results = (List<Fsecurity>) getSession()
					.createCriteria("com.ruizton.main.model.Fsecurity").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fsecurity instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Fsecurity as model where model."
					+ propertyName + "= ?";
