...
public class FlimittradeDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FlimittradeDAO.class);
	// property constants
	...
	public Flimittrade findById(java.lang.String id) {
		log.debug("getting Flimittrade instance with id: " + id);
		try {
			Flimittrade instance = (Flimittrade) getSession().get(
					"com.ruizton.main.model.Flimittrade", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Flimittrade> findByExample(Flimittrade instance) {
		...
		try {
			List<Flimittrade> results = (List<Flimittrade>) getSession()
					.createCriteria("com.ruizton.main.model.Flimittrade").add(create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Flimittrade instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Flimittrade as model where model."
					+ propertyName + "= ?";
	...
	public Flimittrade merge(Flimittrade detachedInstance) {
		...
		try {
			Flimittrade result = (Flimittrade) getSession().merge(
					detachedInstance);
