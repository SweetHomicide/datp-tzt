...
public class EmailvalidateDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(EmailvalidateDAO.class);
	// property constants
	...
	public Emailvalidate findById(java.lang.Integer id) {
		...
		try {
			Emailvalidate instance = (Emailvalidate) getSession().get(
					"com.ruizton.main.model.Emailvalidate", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Emailvalidate> findByExample(Emailvalidate instance) {
		...
		try {
			List<Emailvalidate> results = (List<Emailvalidate>) getSession()
					.createCriteria("com.ruizton.main.model.Emailvalidate").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Emailvalidate instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Emailvalidate as model where model."
					+ propertyName + "= ?";
	...
	public Emailvalidate merge(Emailvalidate detachedInstance) {
		...
		try {
			Emailvalidate result = (Emailvalidate) getSession().merge(
					detachedInstance);
	...
	public Emailvalidate findByUidUuid(String uid,String uuid,int type){
	...
	public List<Emailvalidate> canSendFindPwdMsg(
			String fid,
			String ev_id,
			String newuuid
			){
