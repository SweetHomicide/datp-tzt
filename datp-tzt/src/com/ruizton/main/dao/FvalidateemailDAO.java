...
public class FvalidateemailDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FvalidateemailDAO.class);
	// property constants
	...
	public Fvalidateemail findById(java.lang.String id) {
		log.debug("getting Fvalidateemail instance with id: " + id);
		try {
			Fvalidateemail instance = (Fvalidateemail) getSession().get(
					"com.ruizton.main.model.Fvalidateemail", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fvalidateemail> findByExample(Fvalidateemail instance) {
		...
		try {
			List<Fvalidateemail> results = (List<Fvalidateemail>) getSession()
					.createCriteria("com.ruizton.main.model.Fvalidateemail")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fvalidateemail instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fvalidateemail as model where model."
					+ propertyName + "= ?";
	...
	public Fvalidateemail merge(Fvalidateemail detachedInstance) {
		...
		try {
			Fvalidateemail result = (Fvalidateemail) getSession().merge(
					detachedInstance);
