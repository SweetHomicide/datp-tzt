...
public class FpoolDAO extends HibernateDaoSupport {
	...
	public Map mapFpool(int firstResult, int maxResults, String filter,boolean isFY) {
		...
		try {
			StringBuffer sf = new StringBuffer();
			sf.append("select count(a.fid) c from fpool a left outer join \n");
			sf.append("fvirtualcointype b on a.fvi_type = b.fid \n");
			sf.append(filter+" \n");
			sf.append("group by a.fvi_type \n");
			Query queryObject = getSession().createSQLQuery(sf.toString());
			mapFpool=(Map) queryObject.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).uniqueResult(); //返回值为map集合 且为唯一值（只能返回一条数据） 
