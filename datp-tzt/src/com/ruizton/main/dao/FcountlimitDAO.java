...
public class FcountlimitDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FcountlimitDAO.class);
	// property constants
	...
	public Fcountlimit findById(java.lang.Integer id) {
		...
		try {
			Fcountlimit instance = (Fcountlimit) getSession().get(
					"com.ruizton.main.model.Fcountlimit", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fcountlimit> findByExample(Fcountlimit instance) {
		...
		try {
			List<Fcountlimit> results = (List<Fcountlimit>) getSession()
					.createCriteria("com.ruizton.main.model.Fcountlimit").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fcountlimit instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Fcountlimit as model where model."
					+ propertyName + "= ?";
	...
	public Fcountlimit merge(Fcountlimit detachedInstance) {
		...
		try {
			Fcountlimit result = (Fcountlimit) getSession().merge(
					detachedInstance);
