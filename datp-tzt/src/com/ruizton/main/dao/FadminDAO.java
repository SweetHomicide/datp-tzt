...
public class FadminDAO extends HibernateDaoSupport {
	...
	public List<Fadmin> findByExample(Fadmin instance) {
		...
		try {
			List<Fadmin> results = (List<Fadmin>) getSession()
					.createCriteria("com.ruizton.main.model.Fadmin")
	...
	public Map<Integer,Double> getSQLValue1(String filter){
		...
		if(list != null && list.size() >0 && list.get(0) != null){
			for(int i=0;i<list.size();i++){
				...
				map.put(Integer.parseInt(o[0].toString()), Double.valueOf(o[1].toString()));
			}
		}
		return map;
	}
