...
/**
	 ...
	 * @see com.ruizton.main.model.Farticle
...
public class FarticleDAO extends HibernateDaoSupport  {
    ...
    public List<Farticle> findByExample(Farticle instance) {
        ...
        try {
            List<Farticle> results = (List<Farticle>) getSession()
                    .createCriteria("com.ruizton.main.model.Farticle")
                    .add( create(instance) )
            .list();
	...
	public List<Farticle> findFarticle(String farticletype,int firstResult,int maxResult){
		...
		try {
			String queryString = "from Farticle where farticletype.fid=? order by fisding desc,fcreateDate desc";
	         Query queryObject = getSession().createQuery(queryString);
	...
	public int findFarticleCount(String farticletype){
		...
		try {
			String queryString = "select count(*) from Farticle where farticletype.fid=? order by fcreateDate desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setCacheable(true) ;
			queryObject.setParameter(0, farticletype) ;
