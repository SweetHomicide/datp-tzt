	...
	public List findByProperty(String propertyName, Object value) {
		...
		try {
			String queryString = "from Fentrustlog as model where model."
					+ propertyName + "= ? order by fcreateTime desc";
	...
	public List<Fentrustlog> findLatestSuccessDeal(String coinTypeId,int count){
		...
		try {
			String queryString = "from Fentrustlog where "+
					"fvirtualcointype.fid="+"'"+coinTypeId+"' and "+
					"fcreateTime>? and " +
					"isactive= :s and " +
					"fentrust!=null " +
			...
			queryObject.setParameter(0, new Date(Utils.getTimestamp().getTime()-24*60L*60*1000)) ;
			queryObject.setParameter("s", true) ;
			queryObject.setFirstResult(0);
			queryObject.setMaxResults(count);
			List<Fentrustlog> li=queryObject.list();
			return queryObject.list();			
		} catch (RuntimeException re) {
			log.error("find all failed", re);
			throw re;
