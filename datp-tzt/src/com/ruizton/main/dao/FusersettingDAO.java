...
public class FusersettingDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FusersettingDAO.class);
	// property constants
	...
	public Fusersetting findById(java.lang.Integer id) {
		...
		try {
			Fusersetting instance = (Fusersetting) getSession().get(
					"com.ruizton.main.model.Fusersetting", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fusersetting> findByExample(Fusersetting instance) {
		...
		try {
			List<Fusersetting> results = (List<Fusersetting>) getSession()
					.createCriteria("com.ruizton.main.model.Fusersetting").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fusersetting instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fusersetting as model where model."
					+ propertyName + "= ?";
	...
	public List<Fusersetting> findByFisAutoReturnToAccount(
			Object fisAutoReturnToAccount) {
		return findByProperty(FIS_AUTO_RETURN_TO_ACCOUNT,
				fisAutoReturnToAccount);
	}
	...
	public Fusersetting merge(Fusersetting detachedInstance) {
		...
		try {
			Fusersetting result = (Fusersetting) getSession().merge(
					detachedInstance);
