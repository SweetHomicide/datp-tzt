...
public class FcapitaloperationDAO extends HibernateDaoSupport  {
    ...
    public Fcapitaloperation findById( java.lang.String id) {
	...
	public Map getTotalAmountIn(int type,String fstatus,boolean isToday) {
		...
		sql.append("select fRemittanceType,sum(famount) totalAmount from Fcapitaloperation where fType="+type+" and fstatus in"+fstatus+" \n");
		if(isToday){
	...
	public int findByParamCounts(Map<String, Object> param, String dates, int fstatus) {
		log.debug("finding Fcapitaloperation findByParams");
		try {
			StringBuffer queryString = new StringBuffer("select count(f.fid) from Fcapitaloperation f where f.ftype=2 and f.fcreateTime like "+"'"+dates+"%'"+" and f.fstatus <= "+fstatus+" and ");
	...
	public List<Fcapitaloperation> findByFolderId(String folderIds) {
			...
			StringBuffer queryString = new StringBuffer("select f.fid from Fcapitaloperation f where f.forderId ='" + folderIds+"'");
			Query queryObject = getSession().createQuery(queryString.toString());
			List<Fcapitaloperation> listResult =queryObject.list();
			return  listResult;
	}
}