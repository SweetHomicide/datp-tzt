...
public class FintrolinfoDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FintrolinfoDAO.class);
	// property constants
	...
	public Fintrolinfo findById(java.lang.Integer id) {
		...
		try {
			Fintrolinfo instance = (Fintrolinfo) getSession().get(
					"com.ruizton.main.model.Fintrolinfo", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fintrolinfo> findByExample(Fintrolinfo instance) {
		...
		try {
			List<Fintrolinfo> results = (List<Fintrolinfo>) getSession()
					.createCriteria("com.ruizton.main.model.Fintrolinfo").add(create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fintrolinfo instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Fintrolinfo as model where model."
					+ propertyName + "= ?";
	...
	public Fintrolinfo merge(Fintrolinfo detachedInstance) {
		...
		try {
			Fintrolinfo result = (Fintrolinfo) getSession().merge(
					detachedInstance);
	...
	public List getAllIntrol(int firstResult, int maxResults, String filter,boolean isFY) {
		...
		sf.append("(SELECT fuserid,SUM(fqty) qty from fintrolinfo where fiscny=0 GROUP BY fuserid)as b on a.fid = b.fuserid \n");
		sf.append("LEFT OUTER JOIN \n");
		sf.append("(SELECT fuserid,SUM(fqty) qty from fintrolinfo where fiscny=1 GROUP BY fuserid)as d on a.fid = d.fuserid \n");
		sf.append(")as t  \n");
		sf.append(filter +" \n");
