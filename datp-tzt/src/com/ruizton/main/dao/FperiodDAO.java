...
/**
 	* A data access object (DAO) providing persistence and search support for Fperiod entities.
...
public class FperiodDAO extends HibernateDaoSupport  {
    ...
    public Fperiod findById( java.lang.Integer id) {
        ...
        try {
            Fperiod instance = (Fperiod) getSession()
                    .get("com.ruizton.main.model.Fperiod", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List<Fperiod> findByExample(Fperiod instance) {
        ...
        try {
            List<Fperiod> results = (List<Fperiod>) getSession()
                    .createCriteria("com.ruizton.main.model.Fperiod")
                    .add( create(instance) )
            .list();
    ...
    public List findByProperty(String propertyName, Object value) {
      log.debug("finding Fperiod instance with property: " + propertyName
            + ", value: " + value);
      try {
         String queryString = "from Fperiod as model where model." 
         						+ propertyName + "= ?";
    ...
    public Fperiod merge(Fperiod detachedInstance) {
        ...
        try {
            Fperiod result = (Fperiod) getSession()
    ...
    public List<Fperiod> findAllFperiod(long fromTime,String fvirtualCoinType){
		...
		try {
			String queryString = "from Fperiod where fvirtualcointype.fid = ? and ftime>?";//lwj
	         Query queryObject = getSession().createQuery(queryString);
    ...
    public Fperiod getLastFperiod(Fvirtualcointype fvirtualcointype){
    	...
    	try {
    		String queryString = "from Fperiod where fvirtualcointype.fid=? order by ftime desc";
    		Query queryObject = getSession().createQuery(queryString);
