...
public class FsubscriptionDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FsubscriptionDAO.class);
	// property constants
	...
	public Fsubscription findById(java.lang.String id) {
		log.debug("getting Fsubscription instance with id: " + id);
		try {
			Fsubscription instance = (Fsubscription) getSession().get(
					"com.ruizton.main.model.Fsubscription", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fsubscription> findByExample(Fsubscription instance) {
		...
		try {
			List<Fsubscription> results = (List<Fsubscription>) getSession()
					.createCriteria("com.ruizton.main.model.Fsubscription").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fsubscription instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fsubscription as model where model."
					+ propertyName + "= ?";
	...
	public Fsubscription merge(Fsubscription detachedInstance) {
		...
		try {
			Fsubscription result = (Fsubscription) getSession().merge(
					detachedInstance);
	...
	public List<Fsubscription> list(int firstResult, int maxResults, String filter,boolean isFY) {
		...
		try {
			//String queryString = "from Fsubscription "+filter+" limit "+" "+firstResult+", "+maxResults;
	...
	public int findCount(String sql) {
		...
		try {
			...
			log.error("find by Fsubscription count failed", re);
			throw re;
		}
	}
	...
	public int selectCount(String countSize) {
		try {
		Query query = getSession().createQuery(countSize);
	...
	public Fsubscription findByFviId(String fviId) {
		try {
			String query = "from Fsubscription where fisRMB = 1 and fvirtualcointype.fid = ?";
			Query queryObject = getSession().createQuery(query);
			queryObject.setString(0, fviId);
			Object r = queryObject.uniqueResult();
			log.debug("merge successful");
			return (Fsubscription) r;
		} catch (RuntimeException re) {
			log.error("merge failed", re);
			throw re;
		}
