...
public class FbankinfoWithdrawDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FbankinfoWithdrawDAO.class);
	// property constants
	...
	public FbankinfoWithdraw findById(java.lang.String id) {
		log.debug("getting FbankinfoWithdraw instance with id: " + id);
		try {
			FbankinfoWithdraw instance = (FbankinfoWithdraw) getSession().get(
					"com.ruizton.main.model.FbankinfoWithdraw", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<FbankinfoWithdraw> findByExample(FbankinfoWithdraw instance) {
		...
		try {
			List<FbankinfoWithdraw> results = (List<FbankinfoWithdraw>) getSession()
					.createCriteria("com.ruizton.main.model.FbankinfoWithdraw")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding FbankinfoWithdraw instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from FbankinfoWithdraw as model where model."
					+ propertyName + "= ?";
	...
	public FbankinfoWithdraw merge(FbankinfoWithdraw detachedInstance) {
		...
		try {
			FbankinfoWithdraw result = (FbankinfoWithdraw) getSession().merge(
					detachedInstance);
	...
	public FbankinfoWithdraw findByParam(String fid, String bankNum) {
		log.debug("getting FbankinfoWithdraw instance with userId: "+fid+" and bankNum: " + bankNum);
		try {
			String queryString = "from FbankinfoWithdraw where fuser ='"+fid+"' and fbankNumber ="+bankNum;
			Query queryObject = getSession().createQuery(queryString);
			return (FbankinfoWithdraw)queryObject.list().get(0);
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
		}
