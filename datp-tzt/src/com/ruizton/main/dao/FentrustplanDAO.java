...
public class FentrustplanDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FentrustplanDAO.class);
	// property constants
	...
	public Fentrustplan findById(java.lang.Integer id) {
		...
		try {
			Fentrustplan instance = (Fentrustplan) getSession().get(
					"com.ruizton.main.model.Fentrustplan", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fentrustplan> findByExample(Fentrustplan instance) {
		...
		try {
			List<Fentrustplan> results = (List<Fentrustplan>) getSession()
					.createCriteria("com.ruizton.main.model.Fentrustplan").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Fentrustplan instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from Fentrustplan as model where model."
					+ propertyName + "= ?";
	...
	public Fentrustplan merge(Fentrustplan detachedInstance) {
		...
		try {
			Fentrustplan result = (Fentrustplan) getSession().merge(
					detachedInstance);
	...
	public List<Fentrustplan> findFentrustplan(
			String fuser,
			String fvirtualcointype,
			int[] fstatus,
			int firtResult,
			int maxResult,
			String order){
		...
		try {
			StringBuffer queryString = new StringBuffer("from Fentrustplan where fuser.fid=? " +
	...
	public long findFentrustplanCount(
			String fuser,
			String fvirtualcointype,
			int[] fstatus){
		log.debug("findFentrustplan all Fentrustplan instances");
		try {
			StringBuffer queryString = new StringBuffer("select count(fid) from Fentrustplan where fuser.fid=? " +
					" and fvirtualcointype.fid=? and ");
			queryString.append(" ( ") ;
			if(fstatus!=null){
				for (int i=0;i<fstatus.length;i++) {
	...
	public List<Fentrustplan> list(int firstResult, int maxResults,
