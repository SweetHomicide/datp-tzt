...
public class FroleSecurityDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(FroleSecurityDAO.class);
	...
	public FroleSecurity findById(java.lang.Integer id) {
		...
		try {
			FroleSecurity instance = (FroleSecurity) getSession().get(
					"com.ruizton.main.model.FroleSecurity", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<FroleSecurity> findByExample(FroleSecurity instance) {
		...
		try {
			List<FroleSecurity> results = (List<FroleSecurity>) getSession()
					.createCriteria("com.ruizton.main.model.FroleSecurity").add(create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding FroleSecurity instance with property: "
				+ propertyName + ", value: " + value);
		try {
			String queryString = "from FroleSecurity as model where model."
					+ propertyName + "= ?";
	...
	public FroleSecurity merge(FroleSecurity detachedInstance) {
		...
		try {
			FroleSecurity result = (FroleSecurity) getSession().merge(
					detachedInstance);
	...
	public void deleteByRoleId(String roleId) {
		try {
			...
			query.setParameter(0, roleId);//设定条件参数  
			query.executeUpdate();
		} catch (RuntimeException re) {
			throw re;
		}
