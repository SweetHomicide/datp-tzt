...
public class FvirtualwalletDAO extends HibernateDaoSupport {
	...
	public List<Fvirtualwallet> find(String fuid,int status){
		log.debug("finding all Fvirtualwallet instances");
		try {
	...
	public List<Fvirtualwallet> findX(String fuid,int status,boolean isFY){
		log.debug("finding all Fvirtualwallet instances");
		try {
			String queryString = "from Fvirtualwallet where fuser.fid=? and fvirtualcointype.fstatus=? order by ftotal desc,fvirtualcointype.faddTime desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, fuid) ;
			queryObject.setParameter(1, status) ;
			if(isFY){
	...
	public List<Fvirtualwallet> find(String fuid,int status, int firstResult, int maxResult, boolean isFY){
		log.debug("finding all Fvirtualwallet instances");
		try {
			String queryString = "from Fvirtualwallet where fuser.fid=? and fvirtualcointype.fstatus=? order by ftotal desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, fuid) ;
			queryObject.setParameter(1, status) ;
			if(isFY){
	...
	public Fvirtualwallet findVirtualWallet(String fuid,String fcoinId){
		log.debug("finding all Fvirtualwallet instances");
		try {
	...
	public BigDecimal getTotalQty(String vid) {
		List<Map> all = new ArrayList();
		StringBuffer sql = new StringBuffer();
	...
	public List<Fvirtualwallet> findByName(String fuid, int status, String keyWord, int firstResult, int maxResult,
			boolean isFY) {
		log.debug("finding byName Fvirtualwallet instances");
		try {
			String queryString = "from Fvirtualwallet where fuser.fid=? and fvirtualcointype.fstatus=? and fvirtualcointype.fname  like '%"+keyWord+"%' order by ftotal desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, fuid) ;
			queryObject.setParameter(1, status) ;
			if(isFY){
