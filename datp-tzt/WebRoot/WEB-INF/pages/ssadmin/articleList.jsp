		...
		<ul class="toolBar">
			...
			<shiro:hasPermission name="ssadmin/updateArticle.html">
				...
				<li><a class="edit"
					...
					title="确定要操作吗?"><span>置顶/取消</span> </a></li>
			</shiro:hasPermission>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="60" orderField="fkeyword"
					<c:if test='${param.orderField == "fkeyword" }'> class="${param.orderDirection}"  </c:if>>关键词</th>
				<th width="60">阅读数</th>	
		...
		<tbody>
			<c:forEach items="${articleList}" var="article" varStatus="num">
				<tr target="sid_user" rel="${article.fid}">
					<td>${num.index +1}</td>
					<td>${article.fisding}</td>
					<td>${article.fisout}</td>
					<td>${article.foutUrl}</td>
					<td>${article.ftitle}</td>
					<td>${article.farticletype.fname}</td>
					<td>${article.fkeyword}</td>
					<td>${article.fcount}</td>
