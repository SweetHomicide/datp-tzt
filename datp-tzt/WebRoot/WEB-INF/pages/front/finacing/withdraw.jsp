							...
							<div class="form-group">
								...
								<div class="col-xs-4 text-left">
									...
									value="${ finacing.tenThsPro}" pattern="##.####" maxIntegerDigits="10"
									 />(${finacing.fsymbol})</span>
								</div>
							</div>
							<div class="form-group">
								<div class="col-xs-4 text-right">
									<span for="name">收益率:</span>
								</div>
								<div class="col-xs-4 text-left">
									<span
										style="color: #FB5A5C; ">${ finacing.fproportion}%</span>
								</div>
							</div>
							<div class="form-group">
								<div class="col-xs-4 text-right">
						...
						<div
							...
							<form role="form" class="form-horizontal">
								<div class="form-group">
									...
									value="${wallet.ftotal}" pattern="##.####" maxIntegerDigits="10"
									 />(${finacing.fsymbol})</span>
								</div>
								<div class="form-group">
									<div class="col-xs-4 text-right">
										<span for="finacingCount">支取金额</span>
									</div>
							...
							<button id="btnPay"
								class="btn btn-primary col-xs-offset-1 col-xs-8">提取</button>
						</div>
					</div>
				</div>
			</div>
			<div class="row"
				...
				<ul id="myTab" class="nav nav-tabs">
					<c:forEach items="${list}" var="finac" varStatus="num">
						...
						<c:if test="${ num.index!=0}">
							<li class=""><a href="#${finac.fid}" data-toggle="tab"
								style="font-size: 18px;">${finac.ftitle}</a></li>
						</c:if>
					</c:forEach>
				</ul>
		...
		$(function() {
			$("#btnPay").on(
					...
					function() {
						...
						$.ajax({
							type : "POST",
							url : "/finaWallet/kitingMoney.html",
							dataType : "json",
							data : {
								password : pwd,
								famount : count,
