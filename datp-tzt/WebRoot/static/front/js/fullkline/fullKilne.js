...
Chart.prototype.updateDataAndDisplay2 = function() {
    ...
    this.setTitle();
    ChartManager.getInstance().setCurrentDataSource('frame0.k0','BCCoin'+'.'+this._market_from+'.'+this._time+'.'+this._money_type+'.'+this._contract_unit);
...
ChartManager.prototype.init = function() {
    ...
    DefaultTemplate.loadTemplate("frame0.k0","BCCoin","frame0.order","0.order","frame0.trade","0.trade");
...
MainDataSource.prototype.getTime = function (date) {
	var etime=date;
	return etime;
};
MainDataSource.prototype.update = function (data) {
    ...
    for (i = 0; i < cnt; i++) {
        ...
        this._dataItems.push({
            date:this.getTime(e[0]), open:e[1], high:e[2], low:e[3], close:e[4], volume:e[5]
        });
    }
    return true;
};
...
DarkTheme.prototype.__construct = function() {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#b33120";
...
COrderGraphPlotter.prototype.updateData = function() {
    ...
    if (all.array.length < all.depth_count) return false;
            ...
            success     : function(json) {
                if (GLOBAL_VAR.G_HTTP_REQUEST) {
                    if (this.time != GLOBAL_VAR.time_type || this.market != GLOBAL_VAR.mark_from) {
                        GLOBAL_VAR.TimeOutId = setTimeout(RequestData, 100);
                        return;
                    }
...
var main = function() {
    ...
    window._set_current_url = function(content) {
        GLOBAL_VAR.periodurl = content;
    };
    window._set_current_coinshortName = function(coinshortName) {
        GLOBAL_VAR.coinshortName = coinshortName;
    };
    ...
    GLOBAL_VAR.requestParam = "symbol=4&step=900";
    GLOBAL_VAR.mark_from = '0';
    GLOBAL_VAR.time_type = '900';
    GLOBAL_VAR.limit = '1000';
    refreshTemplate();
    on_size();
    readCookie();
...
function setHttpRequestParam(mark_from, time_type, limit, since){
    var str = "symbol=" + mark_from + "&step=" + time_type+"&rand=" + Math.round(Math.random() * 100);;
    return str;
}
function refreshTemplate() {
    GLOBAL_VAR.chartMgr = DefaultTemplate.loadTemplate(
        "frame0.k0", "BCCoin"+GLOBAL_VAR.mark_from+GLOBAL_VAR.time_type,
...
function getRealData(){
	...
	var url="/kline/fulldepth.html"+"?"+GLOBAL_VAR.requestParam;
		$.get(url,function(data){
			window._set_current_depth(data.depth);
			window.onPushingResponse(data.period.marketFrom, data.period.type, data.period.coinVol, data.period.data);
		},"json");
		realtimeid=window.setTimeout(function() {
			getRealData();
		}, 3000);
}
