var recharge = {
	submitFinForm : function(that) {
		...
		var rechargeType=$('#rechargeTypeCny option:selected').val();//选中的值
	...
	refTenbody : function() {
		...
		var type = document.getElementById("finType").value;
		var rechargeType=$('#rechargeTypeCny option:selected').val();//选中的值
		var url = "/account/refTenbody.html?currentPage=" + currentPage + "&type=" + type + "&fviId="+rechargeType+"&random=" + Math.round(Math.random() * 100);
		jQuery("#recordbody0").load(url, null, function(data) {
			...
			$(".rechargecontinue").on("click", function() {
				var fid = $(this).data().fid;
				recharge.continueRechargeCNY(fid);
			});
		});
	},
	...
	continueRechargeCNY : function(id) {
		var url = "/account/cancelRechargeCnySubmit.html?random=" + Math.round(Math.random() * 100);
		var param = {
			id : id,
			flag:"con"
...
$(function() {
	...
	$("#rechargeTypeCny").change(function() {
		var rechargeType=$('#rechargeTypeCny option:selected').val();//选中的值
		let url="/account/rechargeCny.html?fviId="+rechargeType;
		window.location.href = url;
	});
	$("#recharTypeUnion").change(function(){
		let rechargeType=$('#recharTypeUnion option:selected').val();//选中的值
		let url="/account/rechargeOnline.html?type=2&fviId="+rechargeType;
		window.location.href=url;
	});
	$("#rechargeTypeweChatPay").change(function(){
		let rechargeType=$('#rechargeTypeweChatPay option:selected').val();//选中的值
		let url="/account/weChatPay.html?type=4&fviId="+rechargeType;
		window.location.href=url;
	});
	$("#rechargeTypeAliPay").change(function(){
		let rechargeType=$('#rechargeTypeAliPay option:selected').val();//选中的值
		let url="/account/AliPay.html?type=3&fviId="+rechargeType;
		window.location.href=url;
	});
	//充值方式
	$("#rechargeCny").on("click",function(){
		/*var rechargeType=$('#recharType option:selected').val();//选中的值
		if(rechargeType==""||rechargeType==undefined){
			rechargeType="";
		}*/
		let url="/account/rechargeCny.html?type=1";
		window.location.href = url;
	});
	$("#rechargeCnyUnionPay").on("click",function(){
		/*let rechargeType=$('#recharType option:selected').val();//选中的值
*/		let url="/account/rechargeOnline.html?type=2";
		window.location.href=url;
	});
	$("#rechargeweChatPay").on("click",function(){
		let rechargeType=$('#recharType option:selected').val();//选中的值
		let url="/account/weChatPay.html?type=4";
		window.location.href=url;
	});
	$("#rechargeAliPay").on("click",function(){
//		let rechargeType=$('#recharType option:selected').val();//选中的值
		let url="/account/AliPay.html?type=3";
		window.location.href=url;
	});
