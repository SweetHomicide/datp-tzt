var assets = {
	...
	submitAlipayAddress : function() {
		...
		var phoneCode = 0;
		if (payeeAddr == "") {
			util.showerrortips("binderrortips", "请输入支付宝姓名");
			return;
		}

		if (withdrawAccount == "" || withdrawAccount.length > 200 || withdrawAccount == language["comm.error.tips.62"]) {
			util.showerrortips("binderrortips", "请输入支付宝帐号");
			return;
		}
	...
	detelBankAddress : function(fid) {
		util.showerrortips("", "确定删除？", {
			okbtn : function() {
				$('#alertTips').modal('hide');
				var url = "/user/deleteBankAddress.html?random=" + Math.round(Math.random() * 100);
				var param = {
					fid : fid
				};
	...
	detelCoinAddress : function(fid) {
		util.showerrortips("", "确定删除？", {
			okbtn : function() {
				$('#alertTips').modal('hide');
				var url = "/user/detelCoinAddress.html?random=" + Math.round(Math.random() * 100);
				var param = {
					fid : fid
				};
				jQuery.post(url, param, function(result) {
...
function searchAccount(NUM){
	
	var name=$("#hidlog1").val();
	var symbol=$("#symbol").val();
	var currentPage=$("#currentPage").val();
	location.href="/financial/accountcointype.html?symbol="+symbol+"&searchname="+name+"&currentPage="+NUM;
}
function btnSearchAccount(Num){
	var name=$("#searchAccountName").val();
	$("#hidlog1").val(name);
	searchAccount(Num);
}
