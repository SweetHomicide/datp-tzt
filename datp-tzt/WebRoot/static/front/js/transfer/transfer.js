function submitTransferForm() {
	...
	if (transfertype == null || transfertype == 0) {
		util.showerrortips("transfererrortips", "请选择转账类型");
		return;
	} else {
	...
	if (!reg.test(amount)) {
		util.showerrortips("transfererrortips", "请输入转账数量");
		return;
	} else {
		util.hideerrortips("transfererrortips");
	}
	...
	if (!reg.test(sendAddress)) {
		util.showerrortips("transfererrortips", "请输入转账会员UID");
		return;
	} else {
		util.hideerrortips("transfererrortips");
	}
	...
	if (parseFloat(amount) < 0.01) {
		util.showerrortips("transfererrortips", "最小转账数量0.01个");
		return;
	} else {
		util.hideerrortips("transfererrortips");
	}
	...
	if (tradePwd == "") {
		util.showerrortips("transfererrortips", "交易密码不能为空");
		return;
	} else {
		util.hideerrortips("transfererrortips");
	}
	...
	$.post(url, param, function(result) {
		if (result != null) {
			if (result.code < 0) {
				...
				util.showerrortips("", "提交成功，请耐心等待管理员审核 ！", {
					okbtn : function() {
...
function cancelTransport(id) {
	util.showconfirm(language["comm.error.tips.67"], {
		okbtn : function() {
			...
			var url = "/account/cancelTransport.html?random=" + Math.round(Math.random() * 100);
			var param = {
				fid : id
			};
