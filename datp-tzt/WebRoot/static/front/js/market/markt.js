var markt = {
	...
	submitTradeBtcForm : function(tradeType, flag) {
		...
		if(login=="false"){
			window.location.href="/?forwardUrl=/market.html?symbol="+symbol;
		}
			...
			if (data != "") {
				...
				$.each(data.buys, function(key, value) {
					...
					if (buyele.length == 0) {
						$("#buybox").append("<li id='buy" + key + "' class='list-group-item clearfix'>" +
								...
								"<span class='col-xs-4 padding-clear text-right'>" + util.moneyformat(Number(value.price), coinCount) + "</span>" + 
								"<span class='col-xs-6 padding-left-clear text-right'>" + util.moneyformat(Number(value.amount), 4) + "</span>" + 
								"</span>"+
						...
						buyele.children()[0].children[1].innerHTML = util.moneyformat(Number(value.price), coinCount);
						buyele.children()[0].children[2].innerHTML = util.moneyformat(Number(value.amount), 4);
				...
				$.each(data.sells, function(key, value) {
					...
					if (sellele.length == 0) {
						...
						sellele.children()[0].children[1].innerHTML = util.moneyformat(Number(value.price), coinCount);
						sellele.children()[0].children[2].innerHTML = util.moneyformat(Number(value.amount), 4);
				...
				$.each(data.trades, function(key, value) {
					...
					'<span class="col-xs-3 padding-clear text-right ">' + util.moneyformat(Number(value.price), coinCount) + '</span>' + 
					'<span class="col-xs-5 text-right ' + (value.en_type == 'ask' ? "buycolor" : "sellcolor") + '">' + util.moneyformat(Number(value.amount), 4) + '</span>' + '</li>';
				});
