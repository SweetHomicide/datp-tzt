var security = {
	...
	submitValidatePhoneForm : function(isUpdate) {
		...
		var desc = '';
		if(oldcode.length == 0){
			util.showerrortips('unbindphone-errortips', "原手机短信验证码不能为空");
			return false;
		}
		if (oldcode.indexOf(" ") > -1 || oldcode.length != 6 || !/^[0-9]{6}$/.test(oldcode)) {
			util.showerrortips('unbindphone-errortips', "原手机短信验证码不合法");
			return false;
		}
		if (phone.indexOf(" ") > -1) {
		...
		if(newcode.length == 0){
			util.showerrortips('unbindphone-errortips', "新手机短信验证码不能为空");
			return false;
		}
		if (newcode.indexOf(" ") > -1 || newcode.length != 6 || !/^[0-9]{6}$/.test(newcode)) {
			util.showerrortips('unbindphone-errortips', "新手机短信验证码不合法");
			return false;
		}
		if(imgcode==""){
			util.showerrortips('unbindphone-errortips', "请输入图片验证码");
			return;
		}
	...
	submitBindPhone : function(isUpdate) {
		...
		if (newcode.indexOf(" ") > -1 || newcode.length != 6 || !/^[0-9]{6}$/.test(newcode)) {
			desc = language["comm.error.tips.60"];
			util.showerrortips('bindphone-errortips', desc);
			return false;
		}
	...
	submitbindGoogleForm : function() {
		...
		var desc = '';
		if(code==""){
			util.showerrortips('bindgoogle-errortips',"谷歌验证码不能为空！");
			return;
		}
		if (!/^[0-9]{6}$/.test(code)) {
			desc = language["comm.error.tips.98"];
	...
	submitPwdForm : function(pwdType, istrade) {
		...
		if (istrade) {
			...
			if(originPwd==""){
				util.showerrortips(errorEle, "旧密码为空！");
				return;
			}
		}
		...
		if(newPwd==""){
			util.showerrortips(errorEle, "新密码不能为空！");
			return;
		}
		if(reNewPwd==""){
			util.showerrortips(errorEle, "确认密码不能为空！");
			return;
		}
		...
		jQuery.post(url, param, function(data) {
			if (data.code == 0) {
				if (istrade) {
					util.showerrortips(errorEle, "修改成功，请牢记您的" + (pwdType == 0 ? '登录' : '交易') + "密码！");
				} else {					
					util.showerrortips(errorEle, "设置成功，请牢记您的交易密码！");
				}
				window.setTimeout(function() {
					window.location.href = '/user/security.html';
				}, 1000);
...
$(function() {
	...
	$("#bindemail").on("hidden.bs.modal",function(){
		$("#bindemail-email").val("");
		$("#bindemail-errortips").text("");
		bindemail-errortips
	});
	$("#unbindphone").on("hidden.bs.modal",function(){
		$("#unbindphone-msgcode").val("");
		$("#unbindphone-newphone").val("");
		$("#unbindphone-newmsgcode").val("");
		$("#unbindphone-imgcode").val("");
		$("#unbindphone-errortips").text("");
		
	});
	$("#bindgoogle").on("hidden.bs.modal",function(){
		$("#bindgoogle-topcode").val("");
		$("#bindgoogle-errortips").text("");
	});
	$("#unbindloginpass").on("hidden.bs.modal",function(){
		$("#unbindloginpass-oldpass").val("");
		$("#unbindloginpass-newpass").val("");
		$("#unbindloginpass-confirmpass").val("");
		$("#unbindloginpass-msgcode").val("");
		$("#unbindloginpass-errortips").text("");
		
	});
	$("#unbindtradepass").on("hidden.bs.modal",function(){
		$("#unbindtradepass-oldpass").val("");
		$("#unbindtradepass-newpass").val("");
		$("#unbindtradepass-confirmpass").val("");
		$("#unbindtradepass-msgcode").val("");
		$("#unbindtradepass-errortips").text("");
	});
	$("#bindphone").on("hidden.bs.modal",function(){
		$("#bindphone-newphone").val("");
		$("#bindphone-msgcode").val("");
		$("#bindphone-imgcode").val("");
		$("#bindphone-errortips").text("");
	});
