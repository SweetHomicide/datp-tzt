var util = {
	...
	isPasswordEle : function(pwd,text) {
		var desc = "";
		var c = new RegExp("^(?![0-9]+$)(?![a-zA-Z]+$)[\\S]{6,}$");
		if (pwd == "") {
			desc = text+language["comm.error.tips.17"];
		} else if (pwd.length < 6) {
			desc = text+language["comm.error.tips.3"];
		} else if (pwd.length > 16) {
			desc = text+language["comm.error.tips.18"];
		} else if (!c.test(pwd)) {
			desc = text+language["comm.error.tips.19"];
		}
		return desc;
	},
